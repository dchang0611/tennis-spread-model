...
def fetch_atp_results() -> pd.DataFrame:
    ...
    with sync_playwright() as playwright:
        ...
        current_page = parse_atp_results_text(page.locator("body").inner_text())
        if not current_page.empty:
            frames.append(current_page)
        links = page.locator('a[href*="/en/scores/"]').evaluate_all(
            "els => [...new Set(els.map(e => e.href).filter(h => h.includes('/results')))]"
        )
        for url in links:
